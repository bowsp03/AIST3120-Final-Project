...
from prepro import LABEL_TO_ID, read_conll
...
def train(args, model, train_features, benchmarks):
    ...
    # Inside the train function:
    optimizer = Adam(
        model.parameters(),  # LoRA layers and classifier are trainable
...
def track_resources():
    """Start tracking memory and time for both CPU and GPU"""
    # GPU tracking
    ...
    start_cpu_mem = process.memory_info().rss  # Current process memory
    
    start_time = time.time()
    
    return {
        'start_time': start_time,
        'start_cpu_mem': start_cpu_mem,
        'start_gpu_mem': torch.cuda.memory_allocated() if torch.cuda.is_available() else 0
    }

def print_resource_usage(start_metrics):
    """Print detailed resource usage"""
    # Time measurement
    time_elapsed = time.time() - start_metrics['start_time']
    
    # CPU memory
    current_process = psutil.Process()
    end_cpu_mem = current_process.memory_info().rss
    cpu_mem_used = (end_cpu_mem - start_metrics['start_cpu_mem']) / (1024**2)  # MB
    
    # GPU memory
    if torch.cuda.is_available():
        peak_gpu_mem = torch.cuda.max_memory_allocated() / (1024**3)  # GB
        current_gpu_mem = torch.cuda.memory_allocated() / (1024**3)    # GB
    else:
        peak_gpu_mem = current_gpu_mem = 0

    # Python heap memory
    snapshot = tracemalloc.take_snapshot()
    stat = snapshot.statistics('lineno')
    heap_mem = sum(stat.size for stat in stat) / (1024**2)  # MB
    
    print(f"\n{' Resource Usage ':-^40}")
    print(f"Time elapsed: {time_elapsed:.2f} seconds")
    print(f"CPU memory (process): {cpu_mem_used:.2f} MB")
    print(f"Python heap memory: {heap_mem:.2f} MB")
    if torch.cuda.is_available():
        print(f"Peak GPU memory: {peak_gpu_mem:.2f} GB")
        print(f"Current GPU memory: {current_gpu_mem:.2f} GB")
    
    tracemalloc.stop()

def main():
    metrics = track_resources()
    ...
    parser.add_argument("--alpha", type=float, default=50.0) #alpha_t (gamma)
    parser.add_argument("--alpha_warmup_ratio", default=0.1, type=float) # alpha % in paper
